...
@testset ">> Unpolarizated " begin
    ...
    @testset ">>> $name" for (name, ρ) in [:AxisArray => ρ₀, :DenseArray => ρ₀.data]
        ...
        @test first_energy_derivative(functional, ρ) ≈ expected[:v]
        @test eltype(first_energy_derivative(functional, ρ)) <: DHA.∂ϵ_∂ρ
        @test second_energy_derivative(functional, ρ) ≈ expected[:δv]
        @test eltype(second_energy_derivative(functional, ρ)) <: DHA.∂²ϵ_∂ρ²

        ϵ, ∂ϵ_∂ρ = energy_and_first_derivative(functional, ρ)
        @test ϵ ≈ expected[:ε]
        ...
        with_conv = energy_and_first_derivative(functional, rho)
    ...
    @testset ">>> Scalars" begin
        ...
        @test first_energy_derivative.(:lda_x, ρ) ≈ expected[:v]
        @test first_energy_derivative.(:lda_x, rho) ≈ expected[:v]

        ρ = 1.0u"ρ"
        expected = energy_and_first_derivative(functional, [ρ])
        @test expected.ϵ[1] == @inferred(energy_and_first_derivative(:lda_x, ρ))[1]
        @test expected.∂ϵ_∂ρ[1] == @inferred(energy_and_first_derivative(:lda_x, ρ))[2]
...
@testset ">> Polarized" begin
    ...
    @testset ">>> $name" for (name, ρs) in [:AxisArray => ρ₀ˢ, :DenseArray => ρ₀ˢ.data]
        ...
        ∂ϵ = first_energy_derivative(functional, ρs)
        @test ∂ϵ ≈ vcat(expected[:v_a]', expected[:v_b]')
        δv = vcat(expected[:δv_aa]', expected[:δv_ab]', expected[:δv_bb]')
        @test second_energy_derivative(functional, ρs) ≈ δv

        ϵ, ∂ϵ_∂ρ = energy_and_first_derivative(functional, ρs)
        @test ϵ ≈ expected[:ε]
    ...
    @testset ">>> Scalars" begin
        ...
        a, b = 1.0u"ρ", 1.5u"ρ"
        expected = tuple(first_energy_derivative(XCFunctional(:lda_x, true), [a, b])...)
        @test expected == @inferred first_energy_derivative(:lda_x, a, b)

        expected = energy_and_first_derivative(XCFunctional(:lda_x, true), [a, b])
        @test expected.ϵ[1] == @inferred(energy_and_first_derivative(:lda_x, a, b))[1]
        @test expected.∂ϵ_∂ρ[1] == @inferred(energy_and_first_derivative(:lda_x, a, b))[2]
        @test expected.∂ϵ_∂ρ[2] == @inferred(energy_and_first_derivative(:lda_x, a, b))[3]
